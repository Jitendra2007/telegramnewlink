"""
Ultra-Fast Resolver:
30ms accelerated timers + complete selector suite for Steps 1-4.
"""
...
INIT_ACCELERATOR = """
    ...
    // Accelerate countdown intervals to 30ms!
    const _origSI = window.setInterval;
    window.setInterval = function(fn, delay, ...args) {
        const fastDelay = (delay >= 500) ? 30 : delay;
...
() => {
  ...
  // 1. Final button on Step 4 or UrlShortx
  const finalBtn = document.querySelector('a#final, #rtg-snp21 a, .get-link, a.btn-primary');
  if (finalBtn) {
...
async def main():
    start = time.time()
    print(f"=== TESTING ULTRA-FAST RESOLUTION FOR {TEST_URL} ===")
    ...
    async with async_playwright() as pw:
        ...
        for loop in range(90):  # max 30s
            ...
            try:
                ...
                res = await page.evaluate(ULTRA_FAST_JS)
        ...
        print(f"\n==========================================")
        print(f"ULTRA-FAST RESULT: {found_tg} in {elapsed:.1f}s")
        print(f"==========================================")
        await browser.close()
